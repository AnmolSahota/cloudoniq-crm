...
const shapeMember = (u, leadsMap = {}, visitsMap = {}) => ({
  ...
  assigned_leads: leadsMap[u.id] ?? 0, // leads.assigned_to === u.id
  completed_visits: visitsMap[u.id] ?? 0, // visits.assigned_to === u.id
  closed_deals: 0, // not in API yet — keep 0
});
...
const UserPanel = ({ onClose, onSave, editMember }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const changes = { name: form.name, phone: form.phone };
        ...
        onSave(shapeMember(res.data.data)); // new user starts at 0 counts
      }
...
const MemberCard = ({ member, onToggleStatus, onEdit }) => {
  ...
  return (
    ...
    >
      ...
      {/* Stats — counts derived from leads/visits APIs */}
...
const ManageSalesTeam = () => {
  ...
  // ── Fetch users + leads + visits in parallel, then cross-match ──────────
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        // Build assigned_to → count maps from leads and visits
        const leadsMap = {}; // userId → assigned lead count
        const visitsMap = {}; // userId → completed visit count
  ...
  const displayed =
    filterStatus === "ALL"
      ? team
      : team.filter((m) => m.status === filterStatus);
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex gap-2">
        {[
          { label: "All Members", value: "ALL", count: team.length },
          ...
          >
            {label}
            <span
              className={`text-xs ${filterStatus === value ? "text-white/75" : "text-gray-400"}`}
            >
              ({count})
            </span>
          </button>
