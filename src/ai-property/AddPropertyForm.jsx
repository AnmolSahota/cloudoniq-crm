...
function MonthYearPicker({ value, onChange, error }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className="absolute z-50 mt-2 w-72 bg-white rounded-2xl border border-gray-200
                        shadow-2xl shadow-indigo-100/50 overflow-hidden"
        >
          <button
            type="button"
            onClick={() => {
...
const SelectField = ({
  ...
  <InputField label={label} icon={Icon} error={error} required={required}>
...
export default function AddPropertyForm() {
  ...
  const showForm = Boolean(formData.property_type);

  /* ─── Dynamic amenities list based on selected property type ──
     Falls back to empty array if type not selected yet
     This automatically shows only relevant amenities per type  ── */
  ...
  useEffect(() => {
    try {
      ...
      else toast.error("Please login to add properties");
    } catch {
  ...
  const validate = (
    ...
    if (!data.property_type)
      errs.property_type = "Please select a property type";
    ...
    // ✅ Amenities are OPTIONAL — no validation here
  ...
  const handleTypeSelect = (type) => {
    setFormData((prev) => ({ ...prev, property_type: type, area_sqft: "" }));

    // Reset BHK if switching away from Residential
    if (type !== "Residential") {
      setSelectedBhks([]);
      setCarpetAreas({});
    }

    // ✅ Clear amenities when type changes
    // so previously selected amenities don't carry over to new type
    setSelectedAmenities([]);

    setErrors((prev) => ({ ...prev, property_type: undefined }));
    setTouched((prev) => ({ ...prev, property_type: true }));
  };
  ...
  /* ─── Amenity toggle — no error tracking needed (optional) ── */
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const bhk_configs = isResidential
        ? selectedBhks.map((bhk) => ({
            bhk,
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {/* Property Type */}
              <div data-field-error={errors.property_type ? true : undefined}>
                <SelectField
                  label="Property Type"
                  icon={Building2}
          ...
          {/* ══ AMENITIES ══
              Only shown after property type is selected
              List is filtered per type — all optional             */}
          {showForm && amenitiesList.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 md:p-6">
              <SectionHeader
                icon={Check}
                ...
                {amenitiesList.map((amenity) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isSubmitting}
                      className={`px-3.5 py-2.5 rounded-xl border-2 transition-all font-medium
                                  text-xs flex items-center gap-2 text-left
                        ${
                          active
                            ? "bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200"
                            : "border-gray-200 text-gray-700 hover:border-indigo-400 hover:bg-indigo-50"
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                {images.map((img, i) => (
                  <div key={i} className="relative group aspect-square">
                    ...
                    <div
                      className="absolute bottom-1.5 left-1.5 bg-black/60 text-white
          ...
          <button
            ...
            className={`w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold
                        text-base transition-all flex items-center justify-center
                        gap-2.5 shadow-lg shadow-indigo-200
              ${
